...
public class ExerciseMapper {
    ...
    /**
     * Maps Exercise entity to ExerciseSO without loading nested collections (instructions, tips).
     * Use this for list views to avoid N+1 lazy loading queries.
     */
    public static ExerciseSO toSOWithoutNestedData(Exercise exercise) {
        if (exercise == null) {
            return null;
        }
        ExerciseSO so = new ExerciseSO();
        ...
        so.setDifficulty(exercise.getDifficulty());
        // Skip instructions and tips to avoid N+1 queries
        so.setInstructions(null);
        so.setTips(null);
        so.setImageUrl(exercise.getImageUrl());
        so.setVideoUrl(exercise.getVideoUrl());
        so.setCreatedAt(exercise.getCreatedAt());
    ...
    /**
     * Maps list of Exercise entities to ExerciseSO list without loading nested collections.
     * Use this for list views to avoid N+1 lazy loading queries.
     */
    public static List<ExerciseSO> toSOListWithoutNestedData(List<Exercise> exercises) {
        if (exercises == null) {
            return null;
        }
        return exercises.stream()
                .map(ExerciseMapper::toSOWithoutNestedData)
                .collect(Collectors.toList());
    }
